...
The ATtiny x5 series parts are equipped with a special high speed 8-bit timer, Timer1 (this is very different from the traditional 16-bit timer1 used on the atmega328p and almost every other chip in the 8-bit AVR product line). This timer can be clocked off the system clock, OR from the PLL at 64 mhz or 32mhz - this is then fed into the prescaler. This will impact the frequency of PWM output on Pin 3 - the default prescaler is not changed. See chapter 12 of the datasheet for more information on the high speed timer.
